...
function ImageUpload({
  ...
  if (value) {
    return (
      <div className="relative group rounded-lg overflow-hidden border border-border bg-muted/20">
        ...
        <img src={value} alt={`${label} image`} className="w-full max-h-48 object-contain" />
  ...
  return (
    <div>
      ...
      {sizeError && <p className="mt-1 text-xs text-destructive">Image is too large (max 2 MB).</p>}
    </div>
  )
}
...
function CardSideDisplay({ label, text, image }: { label: string; text: string; image?: string | null }) {
  return (
    <div>
      <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium mb-1">{label}</p>
      {image && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={image} alt={`${label} image`} className="mb-1.5 rounded-md max-h-32 object-contain border border-border" />
      )}
...
function CardForm({
  fields, onChange, onSave, onCancel, saving, error, saveLabel = "Save card",
}: {
  fields: CardFields
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Front</label>
          <Textarea
            autoFocus
          ...
          />
          <ImageUpload label="Front" value={fields.frontImage} onChange={(v) => onChange({ frontImage: v })} />
        </div>
        <div className="space-y-2">
          <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Back</label>
          <Textarea
            ...
            className="resize-none text-sm"
          />
          <ImageUpload label="Back" value={fields.backImage} onChange={(v) => onChange({ backImage: v })} />
        </div>
      </div>
      {error && <p className="text-sm text-destructive">{error}</p>}
      <div className="flex items-center gap-2">
        <Button size="sm" onClick={onSave} disabled={saving}>
          <Check className="h-3.5 w-3.5 mr-1.5" />{saveLabel}
        </Button>
...
export function CardListSubject({
  subjectId,
  deckId,
  cards,
}: {
  ...
  async function handleAdd() {
    ...
    setSaving(true)
    setAddError(null)
    const result = await createCard(subjectId, deckId, {
      front: newCard.front,
      back: newCard.back,
  ...
  async function handleEdit() {
    ...
    setSaving(true)
    setEditError(null)
    const result = await updateCard(subjectId, deckId, editState.id, editState)
    setSaving(false)
  ...
  async function handleDelete(cardId: string) {
    setDeleteError(null)
    const result = await deleteCard(subjectId, deckId, cardId)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="outline" className="gap-2" onClick={() => { setAdding(true); setAddError(null) }}>
      ...
      {cards.length === 0 && !adding ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-12 gap-3 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">Add cards manually, or generate them with AI in Resources.</p>
          </div>
        </div>
      ) : (
        <div className="space-y-2">
                ...
                <div className="flex items-start gap-4 px-4 py-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button
                      ...
                      size="icon"
                      className="h-7 w-7"
                      onClick={() => setEditState({ id: card.id, front: card.front, back: card.back, frontImage: card.frontImage, backImage: card.backImage })}
                    >
